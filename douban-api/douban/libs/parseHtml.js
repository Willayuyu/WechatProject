...
function getLinkingData($) {
    const cheeioScriptJsonLinkingDataEleText = $('head>script[type="application/ld+json"]')[0]?.children[0]?.data || '{}';//可选链操作符( ?. )允许读取位于连接对象链深处的属性的值，而不必明确验证链中的每个引用是否有效。
    try {
...
function getInfo($) {
    ...
    spanEles.forEach(spanEle => {
        if (spanEle.children.length === 1) {
            info[spanEle.children[0].data.remove(':', ' ')] //获取名字
